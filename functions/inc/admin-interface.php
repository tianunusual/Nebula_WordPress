...
function optionsframework_add_admin()
{
    ...
    $of_page = add_submenu_page('themes.php', $themename, '主题选项', 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
    // Default
...
<div class="wrap" id="of_container">
  ...
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">选项已重置</div>
  </div>
...
主题设置选项</h2>
      ...
      <a href="https://hitheme.net" target="_new"><div class="icon-option"> </div></a>
      <div class="clear"></div>
    </div>
    <?php 
...
/functions/images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
    <input type="submit" value="保存所有更改" class="button-primary" />
  </form>
  <form action="<?php 
...
" method="post" style="display:inline" id="ofform-reset">
    ...
    <input name="reset" type="submit" value="重置选项" class="button submit-button reset-button" onclick="return confirm('你确定要还原设置吗？还原后所有设置将恢复到默认状态！');" />
...
function of_load_only()
{
    ...
    {
        echo '<link rel="stylesheet" type="text/css" href="' . get_bloginfo('template_directory') . '/functions/inc/admin-style.css" media="screen" />';
        ?>
...
';
                ...
                $jquery_click_hook = preg_replace('[^A-Za-z0-9]', '', strtolower($value['e_name']));
...
function optionsframework_uploader_function($id, $std, $mod)
{
    ...
    $uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="' . $id . '">上传图片</span>';
    if (!empty($upload)) {
        $hide = '';
    } else {
        $hide = 'hide';
    }
    $uploader .= '<span class="button image_reset_button ' . $hide . '" id="reset_' . $id . '" title="' . $id . '">移除</span>';
    $uploader .= '</div>' . '
';
